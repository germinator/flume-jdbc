...
public class TestAbstractJDBCSink {
  ...
  public void setUp() throws Exception {
    ...
    sink.configure(new Context(ImmutableMap.of("driver", "driver", "url",
        "url", "user", "user", "password", "password", "batchSize", "10")));
  ...
  // Ensure that on the happy path, things seem to clean up and commit in a
  // sensible order.
  @Test
  public void testTransactionalHappyPath() throws EventDeliveryException,
      SQLException, ClassNotFoundException {
    sink.process();
    final InOrder inOrder = inOrder(transaction, connectionManager, connection,
        sink);
  ...
  // Ensure that on some sort of failure, things seem to clean up and roll back
  // in a sensible order.
  @Test
  public void testTransactionalFailurePath() throws Exception {
    when(channel.take()).thenReturn(
        EventBuilder.withBody("", Charset.defaultCharset()));
